...
class _StockManagementPageState extends ConsumerState<StockManagementPage> {
  ...
  Future<void> _handleCommit() async {
    ...
    try {
      ...
      await ref.read(productProvider.notifier).updateStockBulk(deltas);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Stock updated successfully for ${_transactionItems.length} products')),
        );
        setState(() {
        ...
        child: Column(
          children: [
            ...
            const Padding(
              ...
              child: Text('Scan Product Barcode', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            ),
            Expanded(
              child: ClipRRect(
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              action: SnackBarAction(
                label: 'ADD NEW',
                onPressed: () => Navigator.pushNamed(context, '/add-product', arguments: result),
              ),
            ),
          );
        }
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    trailing: const Icon(Icons.add_circle_outline, color: AppTheme.primaryTeal),
                    onTap: () {
                      _addItem(p);
            ...
            Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.shopping_bag_outlined, color: AppTheme.primaryTeal, size: 20),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
