...
import 'package:billify_application/features/analytics/customer_reports/providers/customer_reports_provider.dart';
import 'package:billify_application/features/analytics/customer_reports/services/customer_report_export_service.dart';
...
class CustomerReportsScreen extends ConsumerWidget {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => ref.read(customerReportsProvider.notifier).refresh(),
          ),
        ],
      ),
  ...
  Widget _buildContent(BuildContext context, WidgetRef ref, CustomerReportsState state) {
    return Column(
      children: [
  ...
  Widget _buildSummaryHeader(BuildContext context, CustomerReportSummary summary) {
    return Padding(
  ...
  Widget _buildControlBar(BuildContext context, WidgetRef ref, CustomerReportsState state) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
            child: TextField(
              onChanged: (val) => ref.read(customerReportsProvider.notifier).updateSearch(val),
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (val) {
              ...
              if (val == 'excel') CustomerReportExportService.exportToExcel(state);
            },
            itemBuilder: (context) => [
  ...
  Widget _buildCustomerList(BuildContext context, List<CustomerReportItem> customers) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final balanceColor = isReceivable ? Colors.redAccent : (isPayable ? Colors.green : Colors.grey);
        final balanceLabel = isReceivable ? 'YOU GET' : (isPayable ? 'YOU GIVE' : 'SETTLED');
        ...
        return ListTile(
          ...
          subtitle: Column(
            ...
            children: [
              Text(c.phoneNumber, style: const TextStyle(fontSize: 12, color: Colors.grey)),
              const SizedBox(height: 4),
              Wrap(
                spacing: 6,
                children: [
                  _miniChip('Billed: ₹${c.totalBilled.toStringAsFixed(0)}', Colors.blue.withOpacity(0.12), Colors.blue),
                  _miniChip('Paid: ₹${c.totalPaid.toStringAsFixed(0)}', Colors.green.withOpacity(0.12), Colors.green),
                ],
              ),
            ],
          ),
          trailing: Column(
            ...
            children: [
              Text(
                ...
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: balanceColor),
              ),
              Container(
                ...
                child: Text(balanceLabel,
                    style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: balanceColor)),
              ),
            ],
          ),
          isThreeLine: true,
