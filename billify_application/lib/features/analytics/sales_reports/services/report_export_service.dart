...
class ReportExportService {
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Row(
              mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
              children: [
                _pdfSummaryItem('Total Sales', currencyFormat.format(state.summary.totalSales)),
                _pdfSummaryItem('Total Orders', state.summary.totalOrders.toString()),
                _pdfSummaryItem('Average Bill', currencyFormat.format(state.summary.avgBillValue)),
                _pdfSummaryItem('Items Sold', state.summary.totalItemsSold.toString()),
              ],
            ),
            pw.SizedBox(height: 20),
            ...
            if (state.topProducts.isNotEmpty) ...[
              ...
              pw.TableHelper.fromTextArray(
                ...
                data: state.topProducts.map((p) => [p.name, p.quantity.toString(), currencyFormat.format(p.revenue)]).toList(),
            ...
            pw.Divider(),
            pw.TableHelper.fromTextArray(
              headers: ['Invoice #', 'Date', 'Customer Details', 'Items (Qty)', 'Total', 'Payment'],
              data: state.invoices.map((inv) => [
                inv.id,
  ...
  static Future<void> exportToExcel(SalesReportsState state) async {
    ...
    sheet.appendRow([
      ...
      excel_lib.TextCellValue(dateFormat.format(state.startDate ?? DateTime.now())),
      excel_lib.TextCellValue('To'), 
      excel_lib.TextCellValue(dateFormat.format(state.endDate ?? DateTime.now()))
    ]);
    sheet.appendRow([]);
    sheet.appendRow([excel_lib.TextCellValue('Metric'), excel_lib.TextCellValue('Value')]);
    sheet.appendRow([excel_lib.TextCellValue('Total Sales'), excel_lib.DoubleCellValue(state.summary.totalSales)]);
    sheet.appendRow([excel_lib.TextCellValue('Total Orders'), excel_lib.IntCellValue(state.summary.totalOrders)]);
    sheet.appendRow([excel_lib.TextCellValue('Average Bill'), excel_lib.DoubleCellValue(state.summary.avgBillValue)]);
    sheet.appendRow([excel_lib.TextCellValue('Items Sold'), excel_lib.IntCellValue(state.summary.totalItemsSold)]);
    sheet.appendRow([]);

    // Top Products
    if (state.topProducts.isNotEmpty) {
      sheet.appendRow([excel_lib.TextCellValue('Top Products')]);
      sheet.appendRow([excel_lib.TextCellValue('Product Name'), excel_lib.TextCellValue('Quantity'), excel_lib.TextCellValue('Revenue')]);
    ...
    for (var inv in state.invoices) {
      sheet.appendRow([
        ...
        excel_lib.TextCellValue(inv.items.map((i) => "${i.name} (x${i.quantity})").join(", ")),
